...
Write-Host "Herstart backend (hard): kill 3002 -> start -> health -> open admin (tabs)" -ForegroundColor Cyan
...
# 4) Open admin-tabs (zonder pauze)
$env:__NO_PAUSE = '1'
& "$scriptDir\open-admin-tabs.ps1"
Remove-Item Env:__NO_PAUSE -ErrorAction SilentlyContinue
